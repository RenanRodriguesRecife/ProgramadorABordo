...
const getStatusLabel = st =>  st ? st = 'Ativado' : st = 'Desativado' 


const rederLine = ({name, email, telefone = "Não cadastrado", status},userIndex) => (
...
const renderLines = userData => {
  return userData.map(rederLine).join('');
}
